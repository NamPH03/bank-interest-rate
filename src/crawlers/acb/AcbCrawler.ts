...
export class AcbCrawler extends BaseBankCrawler {
  readonly bankId = "acb";
  readonly bankName = "ACB";
  ...
  readonly portalUrl = "https://acb.com.vn/lai-suat-tien-gui";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching ACB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "ACB" || b.name?.includes("ACB"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("ACB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for ACB`);
    return rawRates;
  }
}
