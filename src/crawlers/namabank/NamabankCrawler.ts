...
export class NamabankCrawler extends BaseBankCrawler {
  readonly bankId = "namabank";
  readonly bankName = "Nam A Bank";
  ...
  readonly portalUrl = "https://namabank.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching Nam A Bank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "NAB" || b.name?.includes("Nam A Bank") || b.name?.includes("Nam Á"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("Nam A Bank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for Nam A Bank`);
    return rawRates;
  }
}
