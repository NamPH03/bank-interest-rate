...
export class MbCrawler extends BaseBankCrawler {
  readonly bankId = "mb";
  readonly bankName = "MB";
  ...
  readonly portalUrl = "https://www.mbbank.com.vn/fee/interest-rate";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching MB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "MBB" || b.name?.includes("MB Bank") || b.name === "MB");

    if (!bankData || !bankData.interestRates) {
      throw new Error("MB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for MB`);
    return rawRates;
  }
}
