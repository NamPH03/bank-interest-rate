...
export class OcbCrawler extends BaseBankCrawler {
  readonly bankId = "ocb";
  readonly bankName = "OCB";
  ...
  readonly portalUrl = "https://ocb.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching OCB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "OCB" || b.name?.includes("OCB") || b.name?.includes("Phương Đông"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("OCB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for OCB`);
    return rawRates;
  }
}
