...
export class SeabankCrawler extends BaseBankCrawler {
  readonly bankId = "seabank";
  readonly bankName = "SeABank";
  ...
  readonly portalUrl = "https://seabank.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching SeABank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "SSB" || b.name?.includes("SeABank"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("SeABank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for SeABank`);
    return rawRates;
  }
}
