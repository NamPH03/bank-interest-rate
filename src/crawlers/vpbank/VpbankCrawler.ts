...
export class VpbankCrawler extends BaseBankCrawler {
  readonly bankId = "vpbank";
  readonly bankName = "VPBank";
  ...
  readonly portalUrl = "https://www.vpbank.com.vn";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching VPBank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "VPB" || b.name?.includes("VPBank") || b.name?.includes("VP Bank"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("VPBank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for VPBank`);
    return rawRates;
  }
}
