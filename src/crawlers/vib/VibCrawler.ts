...
export class VibCrawler extends BaseBankCrawler {
  readonly bankId = "vib";
  readonly bankName = "VIB";
  ...
  readonly portalUrl = "https://vib.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching VIB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "VIB" || b.name?.includes("VIB"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("VIB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for VIB`);
    return rawRates;
  }
}
