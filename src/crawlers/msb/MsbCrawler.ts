...
export class MsbCrawler extends BaseBankCrawler {
  readonly bankId = "msb";
  readonly bankName = "MSB";
  ...
  readonly portalUrl = "https://msb.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching MSB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "MSB" || b.name?.includes("MSB") || b.name?.includes("Hàng Hải"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("MSB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for MSB`);
    return rawRates;
  }
}
