...
export class VietinbankCrawler extends BaseBankCrawler {
  readonly bankId = "vietinbank";
  readonly bankName = "VietinBank";
  ...
  readonly portalUrl = "https://www.vietinbank.vn";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching VietinBank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "CTG" || b.name?.includes("VietinBank") || b.name?.includes("Vietinbank"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("VietinBank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for VietinBank`);
    return rawRates;
  }
}
