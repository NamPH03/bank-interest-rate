...
export class BidvCrawler extends BaseBankCrawler {
  readonly bankId = "bidv";
  readonly bankName = "BIDV";
  ...
  readonly portalUrl = "https://bidv.com.vn/vn/ca-nhan/tra-cuu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching BIDV rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "BID" || b.name?.includes("BIDV"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("BIDV data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for BIDV`);
    return rawRates;
  }
}
