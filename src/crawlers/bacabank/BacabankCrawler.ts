...
export class BacabankCrawler extends BaseBankCrawler {
  readonly bankId = "bacabank";
  readonly bankName = "Bac A Bank";
  ...
  readonly portalUrl = "https://bac-abank.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching Bac A Bank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "BAB" || b.name?.includes("Bac A") || b.name?.includes("Bắc Á"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("Bac A Bank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for Bac A Bank`);
    return rawRates;
  }
}
