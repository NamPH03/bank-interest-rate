...
export class HdbankCrawler extends BaseBankCrawler {
  readonly bankId = "hdbank";
  readonly bankName = "HDBank";
  ...
  readonly portalUrl = "https://hdbank.com.vn/vi/personal/interest-rate";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching HDBank rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "HDB" || b.name?.includes("HDBank") || b.name?.includes("HD Bank"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("HDBank data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for HDBank`);
    return rawRates;
  }
}
