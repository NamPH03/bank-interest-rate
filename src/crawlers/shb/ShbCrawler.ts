...
export class ShbCrawler extends BaseBankCrawler {
  readonly bankId = "shb";
  readonly bankName = "SHB";
  ...
  readonly portalUrl = "https://shb.com.vn/bieu-lai-suat";

  async fetchRates(): Promise<RawBankRate[]> {
    this.logger.info(`Fetching SHB rates...`);
    ...
    const bankData = data.find((b: any) => b.symbol === "SHB" || b.name?.includes("SHB"));

    if (!bankData || !bankData.interestRates) {
      throw new Error("SHB data not found in bank feed");
    }
    ...
    this.logger.info(`Fetched ${rawRates.length} rates for SHB`);
    return rawRates;
  }
}
